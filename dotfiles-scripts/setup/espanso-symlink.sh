# setup-espanso-symlink.sh — Symlink macOS espanso config to ~/.config/espanso
#
...
setup-espanso-symlink() {
  local DOTFILES_CONFIG="$HOME/.config/espanso"
  local ESPANSO_DEFAULT="$HOME/Library/Application Support/espanso"
  ...
  if [[ -L "$ESPANSO_DEFAULT" ]]; then
    local current_target="$(readlink "$ESPANSO_DEFAULT")"
    ...
    local backup="$ESPANSO_DEFAULT.bak.$(date +%s)"
